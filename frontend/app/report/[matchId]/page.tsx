...
export default function ReportPage() {
  ...
  const matchDuration = stats.meta?.durationMinutes ? `${stats.meta.durationMinutes} min` : '—';
        ...
        <div className="relative z-10 p-12 flex flex-col h-full">
          ...
          >
            ...
            <p className="text-base text-white/80 leading-relaxed font-light">
              The game&apos;s fulcrum tilted primarily towards the {stats.possession.home >= 50 ? 'Home' : 'Away'} side, maintaining <span className="text-primary font-bold">{stats.possession.home >= 50 ? stats.possession.home : stats.possession.away}%</span> possession over <span className="text-white font-bold">{matchDuration}</span>. The Home side generated an xG of <span className="text-accent font-bold">{homeXG.toFixed(2)}</span> from <span className="text-white font-bold">{homeShots}</span> shots, while Away created <span className="text-accent font-bold">{awayXG.toFixed(2)}</span> xG from <span className="text-white font-bold">{awayShots}</span> shots. Pass completion stood at <span className="text-primary font-bold">{homePassAcc}%</span> for Home and <span className="text-danger font-bold">{awayPassAcc}%</span> for Away across {stats.passes.attempted} total attempts, reflecting the transitional intensity typical of five-a-side.
            </p>
          </motion.section>
