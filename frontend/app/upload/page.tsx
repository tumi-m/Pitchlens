...
export default function UploadPage() {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: () => setError("Choose one MP4, WebM or MOV video, up to 500 MB."),
  });
    ...
    try {
      ...
      setError(
        err instanceof Error && err.name === "AbortError"
          ...
          : err instanceof Error ? err.message : "Unable to prepare video.",
      );
      setBusy(false);
    } finally {
      active.current = null;
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          <div>
            ...
            <p className="text-pitch-muted leading-relaxed">
              Watch your match and optionally run hosted detection. Nothing is installed on your machine.
            </p>
          </div>
          <div {...getRootProps()} className={`rounded-2xl border-2 border-dashed p-12 text-center cursor-pointer ${isDragActive ? "border-pitch-green bg-pitch-green/10" : "border-pitch-indigo-soft/50"}`}>
            ...
            <p className="text-sm text-pitch-muted mt-2">{file ? `${formatFileSize(file.size)} · Click to replace` : "Or click to choose · MP4, WebM, MOV · Up to 500 MB"}</p>
          </div>
          ...
          <div className="glass-card p-5 space-y-3">
            <label className="flex gap-3 items-start">
              <input type="checkbox" checked={track} onChange={(e) => { setTrack(e.target.checked); if (e.target.checked) setInference(false); }} disabled={!configured || busy} className="mt-1" />
              <span>
                <span className="font-medium">Track the match (hosted)</span>
                <span className="block text-pitch-muted text-sm mt-1">
                  {configured
                    ? "Sends about 40 frames to Roboflow from this site, then stitches boxes into tracks and reviewable pass/turnover candidates. No local Python."
                    : "Needs ROBOFLOW_API_KEY on the deployed app. You can still tag by hand."}
                </span>
              </span>
            </label>
            <label className="flex gap-3 items-start">
              <input type="checkbox" checked={inference} onChange={(e) => { setInference(e.target.checked); if (e.target.checked) setTrack(false); }} disabled={!configured || busy || track} className="mt-1" />
              <span>
                <span className="font-medium">Six-frame inspection only</span>
                <span className="block text-pitch-muted text-sm mt-1">Quick spot-check. Does not build tracks or events.</span>
              </span>
            </label>
          </div>
          ...
          <p className="text-sm text-pitch-muted">Video stays in this browser. Detection frames go to Roboflow through Pitchlens. Confirm every candidate on the tape.</p>
        </div>
      </main>
    </>
  );
