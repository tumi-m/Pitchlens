      ...
      <main className="min-h-screen pt-20 pb-16 px-4">
        <div className="max-w-6xl mx-auto space-y-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-pitch-white">
                Your Matches
              </h1>
            ...
            <div className="flex flex-wrap gap-3 items-start">
              <ImportReview userId={user?.uid ?? "guest"} />
...
function MatchCard({ match }: { match: Match }) {
  ...
  return (
    <Link
      href={`/dashboard/${match.id}`}
      className="block glass-card p-5 hover:border-pitch-indigo-glow/30 transition-all group"
    >
      <div className="flex items-start justify-between mb-4">
        ...
        >
          <Icon
            ...
            className={cn(
              color,
              match.status === "processing" && "animate-spin",
            )}
          />
        </div>
      ...
      {match.review && (
        <p className="text-sm text-pitch-muted mt-3">
          {Math.floor(match.review.duration / 60)}:
          {String(Math.floor(match.review.duration % 60)).padStart(2, "0")} ·{" "}
      ...
      <div className="mt-3 flex items-center justify-between">
        <p className="text-pitch-muted text-xs">
          {match.createdAt?.toDate
            ? format(match.createdAt.toDate(), "dd MMM yyyy")
            : match.createdAt?.seconds
              ? format(new Date(match.createdAt.seconds * 1000), "dd MMM yyyy")
              : format(new Date(), "dd MMM yyyy")}
        </p>
        {match.status === "completed" && (
          <span className="text-pitch-green text-xs font-medium group-hover:underline">
            View Analysis →
          </span>
        )}
      </div>
...
function EmptyState() {
  return (
    <div className="text-center py-20">
      ...
      <h2 className="text-pitch-white font-semibold text-xl mb-2">
        No matches yet
      </h2>
